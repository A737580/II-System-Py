...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    listbox = ListBox(items=['Элемент 1', 'Элемент 2', 'Элемент 3'], on_selection_change=on_selection_changed)
    
    with ui.row():
        new_item_input = ui.input('Новый элемент')
        ui.button('Добавить', on_click=lambda: (listbox.add_item(new_item_input.value), new_item_input.set_value('')))
        ui.button('Удалить выбранный', on_click=listbox.remove_selected)
        ui.button('Очистить', on_click=listbox.clear)
