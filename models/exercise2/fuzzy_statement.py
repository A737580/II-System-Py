...
class FuzzyStatement:
    ...
    def to_string_fuzzy_set(self) -> str:
        return f"{self.parameter} / {self.truth_degree}"
    ...
    def negative(self) -> FuzzyStatement:
        ...
        return FuzzyStatement(
            self._base_text, category_text, self._parameter, truth_degree
        )

    def conjunction(self, secondStatement: FuzzyStatement, type: str) -> FuzzyStatement:
    ...
    def disjunction(self, secondStatement: FuzzyStatement, type: str) -> FuzzyStatement:
        new_truth_degree: float = 0.0
        match type:
            case "алгебраическая сумма":
                new_truth_degree = (
                    ...
                    - (self._truth_degree * secondStatement.truth_degree)
                )
            case "граничная сумма":
                new_truth_degree = min(
                    1, self._truth_degree + secondStatement.truth_degree
                )
            case "драстическая сумма":
                if self._truth_degree == 0.0:
                    new_truth_degree = secondStatement.truth_degree
                    ...
                    new_truth_degree = 1
            case _:
                new_truth_degree = max(self._truth_degree, secondStatement.truth_degree)
        new_base = f"{self._base_text} {self._category_text} ИЛИ {secondStatement.base_text} {secondStatement.category_text}"
        return FuzzyStatement(new_base, "", 0.0, new_truth_degree)
